...
import { mountTelephonyRoutes } from './telephonyRoutes.js';
...
export function createApp(config, fetchImpl = fetch) {
  ...
  app.get('/healthz', (_req, res) => {
    res.json({
      status: 'ok',
      service: 'client-integration-test-backend',
      packages: {
        agents: '@awaazlabs-uva/agents (npm)',
        telephony: '@awaazlabs-uva/telephony (npm)',
      },
    });
  });
  ...
  // --- Telephony SDK (npm @awaazlabs-uva/telephony) ---
  mountTelephonyRoutes(app, config);
