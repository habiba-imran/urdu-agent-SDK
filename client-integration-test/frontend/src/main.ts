...
import { mountTelephonyPanel } from './telephonyPanel';
import './style.css';
...
const backendOrigin = (() => {
  try {
    return new URL(envDefaults.sessionEndpoint || 'http://localhost:3100/api/voice/session').origin;
  } catch {
...
appRoot.innerHTML = `
  <main class="shell">
    <header class="header">
      ...
      <h1>Client integration test</h1>
      <p>
        Fresh-client smoke for <code>npm install @awaazlabs-uva/voice</code>,
        <code>@awaazlabs-uva/agents</code>, and <code>@awaazlabs-uva/telephony</code>
        (host backend). Browser voice + Telnyx numbers / inbound / outbound.
      </p>
...
mountTelephonyPanel({
  backendOrigin,
  getAgentId: () => requireEl<HTMLInputElement>('#agent-id').value,
});
render();
