...
app.innerHTML = `
  <main class="shell">
    <header class="header">
      <h1>UVA Demo</h1>
      <p>
        Browser client for <code>@awaazlabs-uva/voice</code>. Session minting goes through
        <code>demo-app/backend</code> — never call the control plane from the browser.
      </p>
    </header>
    ...
    <section class="panel">
      ...
      <p class="hint">Uses the local SDK package from <code>sdk/</code> (file dependency).</p>
    </section>
...
function bindAgent(client: AwaazLabsUvaVoice): void {
  ...
  client.on('disconnected', () => setStatus('idle'));
