...
class OutboundTrunkDb:
    ...
    def execute(self, query: str, params: tuple[Any, ...] = ()):  # noqa: C901 - compact SQL fake
        ...
        if "select distinct e164_number" in sql and "from telephony_phone_numbers" in sql:
            return FakeCursor(rows=[(number,) for number in self._eligible_numbers(params[0], params[1])])
    ...
    def _existing_trunk_row(self, tenant_id: str, profile_id: str):
        trunk = next(
            (
                ...
                and item["platform_status"] in {"pending_verification", "testing", "active"}
            ),
            None,
        )
        if not trunk:
    ...
    def _insert_trunk(self, params: tuple[Any, ...]):
        tenant_id, _connection_id, _sip_id, profile_id, provider_id, provider_status = params
        trunk = {
    ...
    def _update_trunk(self, params: tuple[Any, ...]):
        ...
        trunk = next(item for item in self.trunks if item["tenant_id"] == tenant_id and item["id"] == record_id)
...
def service_for(db: OutboundTrunkDb, livekit: CapturingLiveKitClient) -> TelephonyService:
...
def test_configure_outbound_trunk_excludes_disabled_released_and_deleted_numbers():
    db = OutboundTrunkDb([
        ...
        managed_number("+14155550123", provider_number_id="pn_released", provisioning_status="released"),
        managed_number("+14155550124", provider_number_id="pn_deleted", deleted_at="2026-08-01T00:00:00Z"),
    ...
    assert livekit.outbound_calls == [("conn_a", "sip.telnyx.example", ["+14155550125"])]
...
def test_configure_outbound_trunk_excludes_cross_tenant_numbers():
    ...
    assert livekit.outbound_calls == [("conn_a", "sip.telnyx.example", ["+14155550123"])]
...
def test_repeated_outbound_trunk_configuration_reuses_existing_record():
    ...
    assert first["outbound_trunk_id"] == second["outbound_trunk_id"] == "lk_tr_out_conn_a"
    assert len(db.trunks) == 1
