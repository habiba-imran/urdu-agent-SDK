        ...
        if "update telephony_phone_numbers" in sql and "provider_number_id = %s" in sql:
            ...
            for number in self.phone_numbers:
                if (
                    ...
                    and (number["provider_number_id"] is None or number["provider_number_id"] == provider_number_id_match)
                ):
                    number.update(
                        {
...
def test_purchase_number_reconciles_pending_order_into_managed_number():
    ...
    service = TelephonyService(
        db_conn=db,
        telnyx_client_factory=lambda api_key, mock_mode: ReconcilingTelnyxClient(api_key, mock_mode),
