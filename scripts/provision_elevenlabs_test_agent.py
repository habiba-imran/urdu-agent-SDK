#!/usr/bin/env python3
"""Provision a throwaway English/ElevenLabs test agent for Phase 6d's live smoke test
...
'elevenlabs' with the seeded voice (migration 0019) — the provider under test. Like the Cartesia
test, this one's TTS output IS real English audio through the actual vendor under test.

    python scripts/provision_elevenlabs_test_agent.py            # dry-run: prints the plan
    python scripts/provision_elevenlabs_test_agent.py --commit   # actually seed the dev DB
"""
...
def main() -> int:
    ...
    with psycopg.connect(**conn_kwargs(), autocommit=True) as c:
        c.execute(
            ...
            "values (%s, 'elevenlabs-en-test', %s, 2, %s)",
        ...
        c.execute(
            ...
            insert into agents (
                ...
                %s, %s, 'ElevenLabs EN Test', %s, 'elevenlabs-default', 'gemini-2.5-flash',
                'en', 'deepgram', 'nova-3', %s,
                'gemini', %s, 'elevenlabs', 'elevenlabs-default', %s
            )
            """,
            (aid, tid, _PROMPT, Jsonb({}), Jsonb({}), Jsonb({})),
        )
    print(
        "\nSEEDED to dev DB (agent_language=en, tts_provider=elevenlabs). "
