#!/usr/bin/env python3
"""Provision a throwaway English/Rime test agent for Phase 6f's live smoke test
...
reused so this test also incidentally re-exercises them), tts_provider='rime' with the seeded
voice (migration 0021) — the provider under test. Real English audio through the actual vendor
under test, not a stand-in.

    python scripts/provision_rime_test_agent.py            # dry-run: prints the plan
    python scripts/provision_rime_test_agent.py --commit   # actually seed the dev DB
"""
    ...
    with psycopg.connect(**conn_kwargs(), autocommit=True) as c:
        ...
        c.execute(
            ...
            insert into agents (
                ...
                %s, %s, 'Rime EN Test', %s, 'rime-arcana-astra', 'gemini-2.5-flash',
                'en', 'deepgram', 'nova-3', %s,
                'gemini', %s, 'rime', 'rime-arcana-astra', %s
            )
            """,
            (aid, tid, _PROMPT, Jsonb({}), Jsonb({}), Jsonb({})),
        )
    print(
        "\nSEEDED to dev DB (agent_language=en, tts_provider=rime). "
