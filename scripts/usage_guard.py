...
BUDGETS = {  # docs/30-GUIDE-FREE-TIER.md §3-4. HARD limits from vendor free tiers.
    ...
    "livekit_agent_min":{"limit": 1000,  "note": "LiveKit Build HARD CAP — calls FAIL past it"},
...
def main():
    u = load(); report = "--report" in sys.argv; bad = False
    print(f"{'provider':<20}{'used':>10}{'limit':>10}{'left':>10}  note")
    for k, b in BUDGETS.items():
        used = u.get(k, 0); left = b["limit"] - used
    ...
    if bad and not report:
        ...
        print("Do NOT rotate accounts (ToS + it risks the Uplift relationship you need for H9).", file=sys.stderr)
